def process_forklift_area(lines: list[str]):
    ...
    return accessible, new_level_line
...
def do_actual_content_processing(file_contents):
    new_contents = []
    accessible = 0
    for line_index in range(len(file_contents)):
        ...
        result = process_forklift_area(lines)
        accessible_in_line = result[0]
        new_line = result[1]
        new_contents.append(new_line)
        accessible += accessible_in_line
    return accessible, new_contents

def process_file(file_name: str):
    file_contents = open(file_name, 'r').read()
    file_contents = split_contents(file_contents)

    removed, new_contents = do_actual_content_processing(file_contents)
    total_removed = removed

    while True:
        # process next iteration of contents
        removed, new_contents = do_actual_content_processing(new_contents)
        total_removed += removed
        if removed == 0:
            break
